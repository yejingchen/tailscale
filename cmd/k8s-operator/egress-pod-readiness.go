...
// condition is set.
// The ProxyGroup StatefulSet updates are rolled out in such a way that no Pod is restarted, before the previous Pod is
// marked as ready, so ensuring that the Pod does not get marked as ready when it is not yet able to route traffic for
// egress service prevents downtime during restarts caused by no available endpoints.
...
// replicas for the ProxyGroup and checks if the received response is healthy response from the Pod being reconciled.
//
...
func (er *egressPodsReconciler) Reconcile(ctx context.Context, req reconcile.Request) (res reconcile.Result, err error) {
	...
	hasLocalAddrPort, err := userHasConfiguredLocalAddrPort(ctx, er.Client, proxyGroupName, er.defaultProxyClass)
	if err != nil {
		return res, fmt.Errorf("error checking if user has configured TS_LOCAL_ADDR_PORT: %w", err)
	}
	if hasLocalAddrPort {
		l.Info("User has configured TS_LOCAL_ADDR_PORT, skipping healthcheck")
		return res, er.setPodReady(ctx, pod, l)
	}
