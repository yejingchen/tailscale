...
// There are three ways a Darwin binary can be run: as the Mac App Store or notarized (macsys) binary,
// or a separate CLI (tailscale) binary that somebody built or downloaded.
//
// The appstore and notarized binaries can communicate directly via XPC with the NEPacketTunnelProvider
// managed tailscaled process and must always call SetCredentials directly.  A built/downloaded CLI binary
// will not be managing tailscaled directly and must source the credentials from a 'sameuserproof' file
// which will be written in the supplied sharedDir when tailscaled calls InitListenerDarwin.

// localTCPPortAndTokenDarwin returns the localhost TCP port number and auth token
// either generated, or sourced from the tailscaled process.
...
// SetCredentials sets an explicit token and port used to authenticate safesocket.
// This is only used by the CLI code in the macos Tailscale.app.
